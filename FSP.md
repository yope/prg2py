...
C64 BASIC does not support ELSE, so we must use GOTO to handle both branches. Unreachable lines are collapsed.
...
def main():
    ...
    while True:
        if state == "init":
            # Original: 10 REM Initialize
            state = "line_10"

        # Line 10 executes as sequential code (not a jump target)
        if state == "line_10":
            # Original: 20 PRINT "START"
            print("START")
            state = "line_20"

        # Line 20-30 execute sequentially (unreachable, shown here for clarity)
        if state == "line_20":
            # Original: 30 A=10
            A = 10
            state = "line_30"

        # Line 40-60 handle conditional branch
        elif state == "line_40":
            # Original: 50 IF A=B THEN GOTO 80
            if A == B:
                state = "line_80"
            else:
                state = "line_60"

        # Line 60 is unreachable but still needs target
        elif state == "line_60":
            # Original: 60 GOTO 90
            state = "line_90"

        # Line 70 is unreachable (collapsed in example)
        # Line 80 is a target (GOTO destination)
        elif state == "line_80":
            # Original: 80 PRINT "EQUAL"
            print("EQUAL")
            state = "next_state"

        # Line 90 is a target (GOTO destination)
        elif state == "line_90":
            # Original: 90 PRINT "END"
            print("END")
            state = "next_state"
...
C64 BASIC does not support apostrophe as REM abbreviation, only REM keyword. Subroutines use RETURN to return to the GOSUB point.
...
75 GOTO 200
100 REM Subroutine
110 PRINT "INSIDE SUB"
120 RETURN
150 REM More code
200 REM Main cleanup
```
...
def main():
    ...
    while True:
        ...
        # Line 50 executes sequentially (GOSUB is a state transition, not line target)
        if state == "line_50":
            # Original: 50 PRINT "CALLING"
            print("CALLING")
            state = "line_70"

        # Line 70 is a target (GOSUB target)
        elif state == "line_70":
            # Original: 70 GOSUB 100
            state = "line_130"  # Next line after GOSUB
            # Track return position
            state._return_to = "line_75"
            state = "line_100"

        # Lines 100-110 are sequential after GOSUB
        elif state == "line_100":
            # Original: 100 REM Subroutine
            print("INSIDE SUB")
            state = "line_110"

        elif state == "line_110":
            # Original: 110 PRINT "INSIDE SUB"
            state = "line_120"

        elif state == "line_120":
            # Original: 120 RETURN
            state = state._return_to
            delstate._return_to

        # Line 150 is a target (GOTO destination)
        elif state == "line_75":
            state = "line_150"

        # Line 150-200 are sequential
        elif state == "line_150":
            print("More code")
            state = "line_200"
...
"""
C64 BASIC to Python Translator
Original program converted from: simple
"""
...
def main():
    ...
    while True:
        if state == "init":
            # Original: 10 PRINT "START"
            print("START")
            state = "line_20"

        # Line 10 executes sequentially (PRINT is target)
        if state == "line_10":
            # Original: 20 A%=10
            A_i = 10.0
            state = "line_30"

        # Line 20 executes sequentially (assignment)
        if state == "line_20":
            # Original: 30 A$="HELLO"
            A_s = "HELLO"
            state = "line_40"

        # Line 30 executes sequentially (no jump target)
        if state == "line_30":
            # Original: 40 B=20
            B = 20.0
            state = "line_40"
            ...
            # Original: 50 IF A=B THEN GOTO 60
            if A_i == B:
                state = "line_60"
            else:
                state = "line_50"
